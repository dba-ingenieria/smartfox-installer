...
> **Fleet agent (v2.3.0+).** When the selected version ships `setup/agent/`, the installer also installs the fleet update agent, its systemd timer and `cosign`, and prompts once for the station's **fleet token** (written to `/etc/smartfox/agent.env`, root-only; an empty token leaves the agent idle). From then on the station updates itself from the fleet manifest inside its maintenance window and rolls back on failure — this script becomes the one-time bootstrap / last manual update. **Recording resumes on its own after an update**: the enable flag is no longer removed by `--update` (only `--cal` removes it). Tokens and rollout commands live in the private `smartfox-fleet` repo.
